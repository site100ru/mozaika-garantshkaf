...
<div class="modal fade" id="calculatePriceWithoutDownloadModal" tabindex="-1" aria-labelledby="calculatePriceWithoutDownloadLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered">
		<form method="post" action="<?php echo get_stylesheet_directory_uri(); ?>/mails/gal-callback.php" class="modal-content" enctype="multipart/form-data">
				...
				<div class="row pb-2">
					<div class="col-12">
						<p class="title"><input type="hidden" name="product" id="modalProductName">
							<span id="modalProductDisplay"></span>
						</p>
					</div>
					<div class="col-12 mb-0">
						<textarea type="text" rows="3" name="mes" class="form-control" placeholder="При желании укажите форму, размеры, материалы или другую информацию"></textarea>
					</div>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="<?php echo get_stylesheet_directory_uri(); ?>/mails/callback.php">
					<p><input type="text" name="name" class="form-control" placeholder="Ваше имя" /></p>
					<p><input type="text" id="phone_mask_measurer" name="tel" class="form-control telMask" placeholder="Ваш телефон" required /></p>

					<input type="hidden" id="g-recaptcha-response-callback-measurer" name="g-recaptcha-response">
					<button type="submit" class="btn btn-danger" style="width: 100%">Отправить</button>
				</form>
			</div>
		</div>
...
<script>
	grecaptcha.ready(function () {
		grecaptcha.execute('6LdV1IcUAAAAADRQAhpGL8dVj5_t0nZDPh9m_0tn', { action: 'action_name' }).then(function (token) {
			...
			// Лид-форма на странице shkafy-cupe-new
			if (document.getElementById('g-recaptcha-response-order-2-home')) {
				document.getElementById('g-recaptcha-response-order-2-home').value = token;
			}

			// Форма «Уже есть проект» на странице shkafy-cupe-new
			if (document.getElementById('g-recaptcha-response-recalc')) {
				document.getElementById('g-recaptcha-response-recalc').value = token;
			}
			...
			// Модалка «Вызов замерщика»
			if (document.getElementById('g-recaptcha-response-callback-measurer')) {
				document.getElementById('g-recaptcha-response-callback-measurer').value = token;
			}
