<?php
/**
 * Template Part: Квиз — новый шаблон (для shkafy-cupe-new.php)
 *
 * Чистая структура шагов + собственная навигация/сбор ответов (скрипт ниже).
...
{
	$step_num = (int) substr($step['id'], strrpos($step['id'], '-') + 1); // 2..6
	?>
<div class="quiz-step" id="<?php echo esc_attr($step['id']); ?>">
	<h3 class="quiz-section-subtitle mb-4 text-start"><span><?php echo esc_html($step['num']); ?></span> <?php echo esc_html($step['title']); ?></h3>
		...
		<div class="quiz-new__wrapper">
			<div class="row g-0">
				<div class="col-xl-9 quiz-new__steps">
					...
					<?php
				// Шаги 1-2 (тип, материал) — первый шаг с неактивной кнопкой «Назад»
				foreach ($image_steps as $idx => $step) {
					quiz_new_image_step($step, $u, $idx === 0);
				}
				?>

					<!-- Шаг 3 — ширина (checkbox) -->
...
<script>
	(function () {
		...
		function collect(id) {
			if (id === 'question-2' || id === 'question-3' || id === 'question-5' || id === 'question-6') {
				var num = id.split('-')[1];
